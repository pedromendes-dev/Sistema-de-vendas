...
>(
  ...
  return memoizee(fn, {
    ...
    normalizer:
      options?.keyGenerator || (((args: any) => JSON.stringify(args)) as any),
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      try {
        ...
        if (duration > 100) {
          // Log slow operations
          console.warn(
            `[PERF] ${name}.${propertyKey} took ${duration.toFixed(2)}ms`
          );
        }
        ...
        const duration = performance.now() - start;
        console.error(
          `[PERF] ${name}.${propertyKey} failed after ${duration.toFixed(2)}ms`
        );
        throw error;
      }
    };
