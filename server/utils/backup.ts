...
export class BackupManager {
  ...
  async listBackups(): Promise<
    Array<{ filename: string; size: string; date: Date }>
  > {
    await this.ensureBackupDirectory();
...
`;
    ...
    attendantsData.forEach((attendant) => {
      sql += `INSERT INTO attendants (id, name, "imageUrl", earnings) VALUES (${
        attendant.id
      }, '${attendant.name.replace(/'/g, "''")}', '${attendant.imageUrl.replace(
        /'/g,
        "''"
      )}', '${attendant.earnings}');\n`;
    });
    ...
    salesData.forEach((sale) => {
      ...
      const clientPhone = sale.clientPhone
        ? `'${sale.clientPhone.replace(/'/g, "''")}'`
        : "NULL";
      const clientEmail = sale.clientEmail
        ? `'${sale.clientEmail.replace(/'/g, "''")}'`
        : "NULL";
      const clientAddress = (sale as any).clientAddress
        ? `'${(sale as any).clientAddress.replace(/'/g, "''")}'`
        : "NULL";
      sql += `INSERT INTO sales (id, "attendantId", value, "createdAt", "clientName", "clientPhone", "clientEmail", "clientAddress") VALUES (${
        sale.id
      }, ${sale.attendantId}, '${sale.value}', '${
    ...
    goalsData.forEach((goal) => {
      const description = goal.description
        ...
        : "NULL";
      sql += `INSERT INTO goals (id, "attendantId", title, description, "targetValue", "currentValue", deadline, "isActive", "createdAt") VALUES (${
        goal.id
      }, ${goal.attendantId}, '${goal.title.replace(
        /'/g,
        "''"
      )}', ${description}, '${goal.targetValue}', '${goal.currentValue}', '${
    ...
    achievementsData.forEach((achievement) => {
      const description = achievement.description
        ...
        : "NULL";
      sql += `INSERT INTO achievements (id, "attendantId", title, description, points, "badgeColor", "unlockedAt") VALUES (${
        achievement.id
      }, ${achievement.attendantId}, '${achievement.title.replace(
        /'/g,
        "''"
      )}', ${description}, ${(achievement as any).points}, '${
    ...
    notificationsData.forEach((notification) => {
      const message = notification.message
        ...
        : "NULL";
      sql += `INSERT INTO notifications (id, type, title, message, "isRead", "createdAt") VALUES (${
        notification.id
      }, '${notification.type}', '${notification.title.replace(
        /'/g,
        "''"
      )}', ${message}, ${notification.isRead}, '${notification.createdAt}');\n`;
    });

    sql += "\n-- Insert Leaderboard\n";
    leaderboardData.forEach((entry) => {
      sql += `INSERT INTO leaderboard (id, "attendantId", points, "salesStreak", "lastSaleDate", "updatedAt") VALUES (${
        entry.id
      }, ${entry.attendantId}, ${(entry as any).points}, ${
        (entry as any).salesStreak
      }, '${(entry as any).lastSaleDate}', '${entry.updatedAt}');\n`;
    });
