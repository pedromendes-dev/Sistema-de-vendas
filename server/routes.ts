...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/attendants/:id", async (req, res) => {
    try {
      ...
      const updatedAttendant = await storage.updateAttendant(id, {
        name,
        imageUrl,
      });
      if (!updatedAttendant) {
  ...
  app.post("/api/sales", async (req, res) => {
    try {
      ...
      const activeGoals = await storage.getActiveGoalsByAttendant(
        validatedData.attendantId
      );
      const updatedAttendant = await storage.getAttendant(
        validatedData.attendantId
      );
      ...
      if (updatedAttendant) {
        ...
        for (const goal of activeGoals) {
          const progress =
            (currentEarnings / parseFloat(goal.targetValue)) * 100;
          ...
          if (progress >= 100 && goal.isActive) {
            ...
            // Update leaderboard
            const leaderboardEntry = await storage.getLeaderboardByAttendant(
              validatedData.attendantId
            );
            const currentPoints = leaderboardEntry
              ? leaderboardEntry.totalPoints
              : 0;
            const currentStreak = leaderboardEntry
              ? leaderboardEntry.currentStreak + 1
              : 1;
            const bestStreak = leaderboardEntry
              ? Math.max(leaderboardEntry.bestStreak || 0, currentStreak)
              : 1;
  ...
  app.get("/api/achievements/attendant/:attendantId", async (req, res) => {
    try {
      const attendantId = parseInt(req.params.attendantId);
      const achievements = await storage.getAchievementsByAttendant(
        attendantId
      );
      res.json(achievements);
    } catch (error) {
  ...
  app.put("/api/achievements/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const { attendantId, title, description, pointsAwarded, badgeColor } =
        req.body;
  ...
  app.get("/api/search", async (req, res) => {
    try {
      const { q, type, dateFrom, dateTo, attendantId, minValue, maxValue } =
        req.query;
      ...
      if (!type || type === "all" || type === "attendants") {
        const attendants = await storage.getAllAttendants();
        results.attendants = attendants
          .filter((a) => {
            if (q && !a.name.toLowerCase().includes(q.toString().toLowerCase()))
              return false;
            if (attendantId && a.id !== parseInt(attendantId.toString()))
              return false;
            return true;
          })
          .map((a) => ({
      ...
      if (!type || type === "all" || type === "sales" || type === "clients") {
        ...
        results.sales = sales
          .filter((s) => {
            ...
            if (
              dateFrom &&
              new Date(s.createdAt) < new Date(dateFrom.toString())
            )
              return false;
            if (dateTo && new Date(s.createdAt) > new Date(dateTo.toString()))
              return false;
            if (
              attendantId &&
              s.attendantId !== parseInt(attendantId.toString())
            )
              return false;
            if (
              minValue &&
              parseFloat(s.value) < parseFloat(minValue.toString())
            )
              return false;
            if (
              maxValue &&
              parseFloat(s.value) > parseFloat(maxValue.toString())
            )
              return false;
      ...
      if (!type || type === "all" || type === "goals") {
        ...
        results.goals = goals
          .filter((g) => {
            if (
              q &&
              !g.title.toLowerCase().includes(q.toString().toLowerCase())
            )
              return false;
            if (
              attendantId &&
              g.attendantId !== parseInt(attendantId.toString())
            )
              return false;
            if (
              dateFrom &&
              new Date((g as any).deadline) < new Date(dateFrom.toString())
            )
              return false;
            if (
              dateTo &&
              new Date((g as any).deadline) > new Date(dateTo.toString())
            )
              return false;
            return true;
          })
      ...
      if (!type || type === "all" || type === "achievements") {
        ...
        results.achievements = achievements
          .filter((a) => {
            if (
              q &&
              !a.title.toLowerCase().includes(q.toString().toLowerCase())
            )
              return false;
            if (
              attendantId &&
              a.attendantId !== parseInt(attendantId.toString())
            )
              return false;
            if (
              dateFrom &&
              new Date((a as any).unlockedAt) < new Date(dateFrom.toString())
            )
              return false;
            if (
              dateTo &&
              new Date((a as any).unlockedAt) > new Date(dateTo.toString())
            )
              return false;
            return true;
          })
