...
export default function NotificationCenter({
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case "urgent":
        return (
          <Badge className="text-xs animate-pulse bg-white text-red-700 border border-red-300">
            Urgente
          </Badge>
        );
      case "high":
        return (
          <Badge className="text-xs bg-white text-red-700 border border-red-300">
            Alta
          </Badge>
        );
      case "medium":
        return (
          <Badge className="text-xs bg-white text-yellow-700 border border-yellow-300">
            Média
          </Badge>
        );
      case "low":
        return (
          <Badge className="text-xs bg-white text-gray-700 border border-gray-300">
            Baixa
          </Badge>
        );
      default:
    ...
    switch (filter) {
      ...
      case "achievements":
        return notifications.filter((n) =>
          ["achievement", "goal", "milestone"].includes(n.type)
        );
      default:
  ...
  const filteredNotifications = filterNotifications(notifications as any[]);
                    ...
                    <p className="text-sm">
                      {filter === "all"
                        ...
                        : `Nenhuma notificação de ${
                            filter === "sales" ? "vendas" : "conquistas"
                          } encontrada`}
                    </p>
                  </div>
                ) : (
