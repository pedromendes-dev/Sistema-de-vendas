...
export default function DashboardPreview({
  ...
  const getWidgetData = (widget: Widget) => {
    switch (widget.config.dataSource) {
      case "/api/attendants":
        return {
          count: (attendants as any[]).length,
          items: attendants,
          label: "Atendentes",
        };
      case "/api/sales":
        return {
          ...
          items: sales as any[],
          label: "Vendas",
          total: (sales as any[]).reduce(
            (sum: number, sale: any) => sum + parseFloat(sale.value || 0),
            0
          ),
        };
      case "/api/goals":
        return {
          count: (goals as any[]).filter((goal: any) => goal.isActive).length,
          items: goals as any[],
          label: "Metas Ativas",
        };
      case "/api/achievements":
        return {
          count: (achievements as any[]).length,
          items: achievements as any[],
          label: "Conquistas",
        };
      default:
  ...
  const getGridClasses = () => {
    ...
    const columnsClass = `grid-cols-1 md:grid-cols-${Math.min(
      layout.columns,
      4
    )} lg:grid-cols-${layout.columns}`;
  ...
  const getSizeClasses = (size: Widget["size"]) => {
    switch (size) {
      ...
      case "full":
        return `col-span-1 md:col-span-${Math.min(
          layout.columns,
          4
        )} lg:col-span-${layout.columns}`;
      default:
        return "col-span-1";
    }
  };
  ...
  const renderWidget = (widget: Widget) => {
    ...
    const colorClass =
      colorSchemeClasses[
        widget.config.colorScheme as keyof typeof colorSchemeClasses
      ] || colorSchemeClasses.blue;
    ...
    switch (widget.type) {
      case "stat":
        return (
          <Card
            key={widget.id}
            className={`${sizeClass} ${colorClass} border-2`}
          >
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                ...
                <div className="opacity-60">
                  {widget.config.dataSource === "/api/attendants" && (
                    <Users size={24} />
                  )}
                  {widget.config.dataSource === "/api/sales" && (
                    <DollarSign size={24} />
                  )}
                  {widget.config.dataSource === "/api/goals" && (
                    <Target size={24} />
                  )}
                  {widget.config.dataSource === "/api/achievements" && (
                    <Trophy size={24} />
                  )}
                </div>
              </div>
                    ...
                    return (
                      <div key={index}>
                        <div className="flex justify-between text-sm mb-2">
                          <span>
                            {item.name || item.title || `Meta ${index + 1}`}
                          </span>
                        ...
                        <div className="w-full bg-muted/30 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full transition-all duration-500 ${
                              widget.config.colorScheme === "green"
                                ? "bg-green-500"
                                : widget.config.colorScheme === "blue"
                                ? "bg-blue-500"
                                : widget.config.colorScheme === "orange"
                                ? "bg-orange-500"
                                : "bg-primary"
                            }`}
      ...
      {isPreview && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p className="text-sm text-blue-700 mt-1">
            Esta é uma prévia de como seu dashboard personalizado aparecerá na
            página inicial.
          </p>
        </div>
      )}

      <div className={getGridClasses()}>{sortedWidgets.map(renderWidget)}</div>
      ...
      {layout.widgets.filter((w) => w.visible).length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            Adicione widgets ou torne alguns visíveis para ver seu dashboard
            personalizado.
          </p>
        </div>
      )}
