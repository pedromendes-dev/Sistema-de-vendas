...
export function BackupManager() {
  ...
  const cleanBackups = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("DELETE", "/api/backup/clean?days=30");
      if (!response.ok) throw new Error("Failed to clean backups");
      return response.json();
    },
    onSuccess: (data) => {
      toast({
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          Proteja seus dados com backups regulares. Recomendamos fazer backup
          semanalmente.
        </p>
      </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {backups[0]
                    ? format(new Date(backups[0].date), "dd/MM/yyyy HH:mm", {
                        locale: ptBR,
                      })
                    : "Nenhum"}
                </p>
                ...
                <p className="text-sm text-muted-foreground">
                  {backups
                    .reduce((acc, b) => {
                      ...
                      return (
                        acc + (b.size.includes("MB") ? sizeNum * 1024 : sizeNum)
                      );
                    }, 0)
                    .toFixed(2)}{" "}
                  KB
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        <span className="flex items-center gap-1">
                          ...
                          {format(new Date(backup.date), "dd/MM/yyyy HH:mm", {
                            locale: ptBR,
                          })}
                        </span>
                        <span className="flex items-center gap-1">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1 text-sm text-muted-foreground">
                <p>
                  • Os backups são salvos automaticamente no servidor em formato
                  JSON
                </p>
                <p>
                  • Recomendamos fazer backup semanalmente ou antes de grandes
                  mudanças
                </p>
                <p>
                  • Backups antigos (mais de 30 dias) podem ser limpos
                  automaticamente
                </p>
                <p>
                  • Para restaurar um backup, entre em contato com o suporte
                  técnico
                </p>
                <p>
                  • Os backups incluem: Atendentes, Vendas, Metas, Conquistas e
                  Notificações
                </p>
              </div>
            </div>
