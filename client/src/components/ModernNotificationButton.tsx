...
export default function ModernNotificationButton() {
  ...
  useEffect(() => {
    if ((unreadNotifications as any[]).length > 0) {
      setAnimateCount(true);
      const timer = setTimeout(() => setAnimateCount(false), 600);
      return () => clearTimeout(timer);
    }
  }, [(unreadNotifications as any[]).length]);
        ...
        <div className="relative">
          ...
          {unreadCount > 0 && (
            <div className="absolute -top-2 -right-2">
              <div
                className={`h-5 min-w-[20px] px-1 text-[10px] font-bold bg-gradient-to-r from-red-500 to-pink-500 text-white border-0 shadow-lg transition-all duration-300 rounded-full flex items-center justify-center ${
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="p-4 bg-gray-100 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-lg text-primary-light">
                    Notificações
                  </h3>
          ...
          <ScrollArea className="max-h-[400px]">
            {(displayNotifications as any[]).length === 0 ? (
                  ...
                  .map((notification: Notification) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {notification.isRead === 0 && (
                          <div
                            className={`absolute top-2 right-2 w-2 h-2 ${style.accentColor} rounded-full animate-pulse`}
                          ></div>
                        )}
                        ...
                        <div className="flex items-start gap-3">
                          {/* Icon */}
                          <div
                            className={`p-2 rounded-lg ${style.bg} ${style.iconColor} flex-shrink-0`}
                          >
                            {style.icon}
                          </div>
          ...
          {(displayNotifications as any[]).length > 0 && (
            <div className="p-3 bg-accent/10 border-t border-border/30 text-center">
              <p className="text-xs text-secondary-light">
                Mostrando {Math.min(displayNotifications.length, 20)} de{" "}
                {displayNotifications.length} notificações
              </p>
            </div>
          )}
        </div>
