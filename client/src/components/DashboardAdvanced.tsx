...
const MetricCard: React.FC<MetricCardProps> = ({
  ...
  const colorClasses = {
    primary:
      "from-primary/10 to-primary/5 border-primary/20 text-primary bg-primary/20",
    success:
      "from-success/10 to-success/5 border-success/20 text-success bg-success/20",
    info: "from-info/10 to-info/5 border-info/20 text-info bg-info/20",
    warning:
      "from-warning/10 to-warning/5 border-warning/20 text-warning bg-warning/20",
  };

  const classes =
    colorClasses[color as keyof typeof colorClasses] || colorClasses.primary;
  const [fromClass, toClass, borderClass, textClass, bgClass] =
    classes.split(" ");
...
export function DashboardAdvanced() {
  ...
  const conversionRate =
    attendants.length > 0
      ? (attendants.filter((a) => parseFloat(a.earnings) > 0).length /
          attendants.length) *
        100
      : 0;
  ...
  const topPerformers = attendants
    .map((att) => {
      ...
      const revenue = attSales.reduce(
        (sum, sale) => sum + parseFloat(sale.value),
        0
      );
      const salesCount = attSales.length;
      const avgTicket = salesCount > 0 ? revenue / salesCount : 0;
      const attGoals = goals.filter(
        (g) => g.attendantId === att.id && g.isActive === 1
      );
  ...
  const customerMetrics = {
    uniqueCustomers: new Set(
      filteredSales.map((s) => s.clientName).filter(Boolean)
    ).size,
    repeatCustomers: (() => {
  ...
  customerMetrics.averageCustomerValue =
  ...
  const categoryDistribution = [
    {
      name: "Produtos",
      value: filteredSales.filter((s) => parseFloat(s.value) < 50).length,
      color: "#10b981",
    },
    {
      name: "Serviços",
      value: filteredSales.filter(
        (s) => parseFloat(s.value) >= 50 && parseFloat(s.value) < 150
      ).length,
      color: "#3b82f6",
    },
    {
      name: "Premium",
      value: filteredSales.filter((s) => parseFloat(s.value) >= 150).length,
      color: "#8b5cf6",
    },
        ...
        <MetricCard
          ...
          trend={
            previousPeriodSales.length > 0
              ? ((averageTicket -
                  previousRevenue / previousPeriodSales.length) /
                  (previousRevenue / previousPeriodSales.length)) *
                100
              : 0
          }
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              <p className="text-2xl font-bold">
                {customerMetrics.uniqueCustomers}
              </p>
            ...
            <div className="text-center">
              <p className="text-2xl font-bold">
                R$ {customerMetrics.averageCustomerValue.toFixed(2)}
              </p>
              <p className="text-sm text-muted-foreground">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {performer.salesCount} vendas | Ticket: R${" "}
                            {performer.avgTicket.toFixed(2)}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-bold">
                          R$ {performer.revenue.toFixed(2)}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {performer.performance.toFixed(1)}% do total
                        </p>
                      </div>
                    </div>
                  ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <ComposedChart
                    data={salesByHour.filter((_, i) => i % 3 === 0)}
                  >
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="hour" />
                    <YAxis yAxisId="left" />
