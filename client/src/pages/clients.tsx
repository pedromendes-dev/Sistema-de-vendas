...
export default function ClientsPage() {
  ...
  const clients = useMemo(() => {
    ...
    sales.forEach((sale) => {
      ...
      if (existing) {
        ...
        if (sale.clientEmail && !existing.email)
          existing.email = sale.clientEmail;
        if (sale.clientAddress && !existing.address)
          existing.address = sale.clientAddress;
      } else {
        clientMap.set(key, {
  ...
  const totalSalesCount = filteredClients.reduce(
    ...
    <div className="container mx-auto px-4 py-8">
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <Label
                htmlFor="search"
                className="text-secondary-light mb-2 block"
              >
                Buscar Cliente
              </Label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredClients.map((client, index) => (
          <Card
            key={index}
            className="bg-secondary-dark border-border hover:border-primary/50 transition-all"
          >
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-secondary-light">
                      Cliente desde{" "}
                      {new Date(client.firstPurchase).toLocaleDateString(
                        "pt-BR"
                      )}
                    </p>
                  </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {client.attendants.map((attendant, idx) => (
                    <Badge
                      key={idx}
                      variant="secondary"
                      className="text-xs bg-white text-gray-800 border-gray-300"
                    >
                      {attendant}
                    </Badge>
                  ))}
