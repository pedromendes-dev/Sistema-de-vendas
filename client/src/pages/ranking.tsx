...
export default function Ranking() {
    ...
    attendants
      ?.map((attendant) => {
        const attendantSales =
          sales?.filter((sale) => sale.attendantId === attendant.id) || [];
  ...
  const getRankBadge = (position: number) => {
    switch (position) {
      case 0:
        return (
          <Badge className="bg-white text-yellow-700 border border-yellow-400 font-bold">
            🥇 1º Lugar
          </Badge>
        );
      case 1:
        return (
          <Badge className="bg-white text-gray-700 border border-gray-400 font-bold">
            🥈 2º Lugar
          </Badge>
        );
      case 2:
        return (
          <Badge className="bg-white text-amber-700 border border-amber-400 font-bold">
            🥉 3º Lugar
          </Badge>
        );
      default:
        return (
          <Badge
            variant="outline"
            className="bg-white text-gray-700 border border-gray-400"
          >
            {position + 1}º Lugar
          </Badge>
        );
      ...
      <main className="px-4 py-6 pb-20 sm:pb-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-secondary-light">
                Classificação dos melhores vendedores
              </p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          {rankingData.map((attendant, index) => (
            <Card
              key={attendant.id}
              className={`${getCardStyle(index)} ranking-card mobile-shadow-lg`}
            >
              <CardContent className="p-6">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="text-lg sm:text-xl font-bold text-primary-light truncate">
                        {attendant.name}
                      </h3>
                  ...
                  <div className="hidden sm:grid sm:grid-cols-4 sm:gap-4 lg:gap-8 sm:flex-1">
                    <div className="text-center">
                      ...
                      <p className="text-xs lg:text-sm text-secondary-light">
                        Faturamento
                      </p>
                    </div>
                    <div className="text-center">
                    ...
                    <div className="text-center">
                      <p className="text-lg lg:text-2xl font-bold text-warning">
                        R$ {attendant.averageTicket.toFixed(2)}
                      </p>
                      <p className="text-xs lg:text-sm text-secondary-light">
                        Ticket Médio
                      </p>
                    ...
                    <div className="text-center">
                      <p className="text-lg lg:text-2xl font-bold text-purple-400">
                        {attendant.completionRate.toFixed(0)}%
                      </p>
                      <p className="text-xs lg:text-sm text-secondary-light">
                        Meta
                      </p>
                  ...
                  <div className="text-right sm:hidden">
                    ...
                    <div className="text-sm text-secondary-light">Total</div>
                  </div>
        ...
        {rankingData.length === 0 && (
          <Card className="bg-card border-border">
            <CardContent className="p-8 text-center">
              <Trophy
                size={48}
                className="mx-auto mb-4 text-secondary-light opacity-50"
              />
              <h3 className="text-lg font-semibold text-primary-light mb-2">
                Nenhum vendedor encontrado
              </h3>
              <p className="text-secondary-light">
                Adicione atendentes para ver o ranking de vendas.
              </p>
            </CardContent>
          </Card>
        )}
