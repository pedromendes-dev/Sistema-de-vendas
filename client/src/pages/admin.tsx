...
export default function Admin() {
  ...
  const [editingAttendant, setEditingAttendant] = useState<Attendant | null>(
    null
  );
  ...
  const [attendantViewMode, setAttendantViewMode] = useState<
    "cards" | "table" | "detailed"
  >("cards");
  const [attendantSearchQuery, setAttendantSearchQuery] = useState("");
  const [attendantSortBy, setAttendantSortBy] = useState<
    "name" | "earnings" | "createdAt"
  >("name");
  const [attendantSortOrder, setAttendantSortOrder] = useState<"asc" | "desc">(
    "asc"
  );
  ...
  const copyAttendantInfo = (attendant: Attendant) => {
    const info = `Nome: ${attendant.name}\nVendas: R$ ${
      attendant.earnings
    }\nCadastrado: ${new Date((attendant as any).createdAt).toLocaleDateString(
      "pt-BR"
    )}`;
...
📅 Cadastrado em: ${new Date((attendant as any).createdAt).toLocaleDateString(
      "pt-BR"
    )}`;
  ...
  const exportAttendantData = async (attendant: Attendant) => {
    ...
    const salesData =
      (sales as any[])?.filter(
        (sale: any) => sale.attendantId === attendant.id
      ) || [];
    ...
    const data = {
      attendant: {
        ...
        created: (attendant as any).createdAt,
      },
      statistics: stats,
  ...
  const toggleAttendantStatus = async (attendantId: number) => {
    try {
      const attendant = (attendants as any[]).find(
        (a: any) => a.id === attendantId
      );
      if (!attendant) return;
      ...
      toast({
        ...
        description:
          attendant.name +
          " está agora " +
          ((attendant as any).status === "active" ? "inativo" : "ativo") +
          ".",
      });
    } catch (error) {
      toast({
    ...
    if (file) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        toast({
          ...
          description:
            "O arquivo deve ter no máximo 5MB. Tente redimensionar a imagem.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (!allowedTypes.includes(file.type)) {
        toast({
          ...
          description:
            "Use apenas imagens nos formatos: JPG, PNG, GIF ou WebP.",
          variant: "destructive",
        });
        return;
      }
  ...
  const filteredAndSortedAttendants = (attendants as any[])
  ...
  const getAttendantStats = (attendant: Attendant) => {
    const attendantSales = (sales as any[]).filter(
      (sale: Sale) => sale.attendantId === attendant.id
    );
    const attendantGoals = (goals as any[]).filter(
      (goal: Goal) => goal.attendantId === attendant.id
    );
    const attendantAchievements = (achievements as any[]).filter(
      (achievement: Achievement) => achievement.attendantId === attendant.id
    );

    return {
      totalSales: attendantSales.length,
      averageSale:
        attendantSales.length > 0
          ? attendantSales.reduce(
              (sum, sale) => sum + parseFloat(sale.value),
              0
            ) / attendantSales.length
          : 0,
      ...
      totalPoints: attendantAchievements.reduce(
  ...
  const createAttendantMutation = useMutation({
    mutationFn: async (data: { name: string; imageUrl: string }) => {
      const response = await apiRequest("POST", "/api/attendants", {
        ...data,
        earnings: "0.00",
      });
  ...
  const updateSaleMutation = useMutation({
    mutationFn: async ({
      ...
      id: number;
      data: { attendantId: number; value: string };
    }) => {
      ...
      <main className="container-universal pb-20 sm:pb-8 pt-4 sm:pt-6 constrain-width">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6 constrain-width">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-universal-2xl font-bold text-primary-light">
                Painel do Gestor
              </h2>
              <p className="text-universal-sm text-secondary-light">
        ...
        <Tabs
          defaultValue="attendants"
          className="space-y-4 constrain-all"
          value={activeTab}
        ...
        >
          ...
          <TabsContent
            value="attendants"
            className="space-universal-md constrain-width"
          >
            ...
            <div className="lg:hidden">
              <Dialog>
                <DialogTrigger asChild>
                  <Card className="bg-gradient-to-r from-success/10 to-info/10 border-success/30 hover:from-success/20 hover:to-info/20 cursor-pointer transition-all duration-300 card-universal constrain-width">
                    <CardContent className="space-universal-lg text-center">
                      ...
                      <h3 className="text-universal-lg font-bold text-primary-light mb-2">
                ...
                <DialogContent className="bg-card border-border text-primary-light card-universal mx-4 constrain-width">
                  <DialogHeader>
                    <DialogTitle className="text-universal-lg text-primary-light">
                      Novo Atendente
                    </DialogTitle>
                  </DialogHeader>
                  <div className="space-universal-md">
                    <div>
                      <Label className="text-universal-sm text-secondary-light">
                        Nome *
                      </Label>
                      <Input
              ...
              <CardContent className="space-y-6">
                <Tabs defaultValue="basic" className="w-full">
                  <TabsList className="grid w-full grid-cols-3 bg-secondary-dark">
                    <TabsTrigger
                      value="basic"
                      className="text-secondary-light data-[state=active]:text-primary-light"
                    >
                      Básico
                    </TabsTrigger>
                    <TabsTrigger
                  ...
                  <TabsContent value="basic" className="space-y-4 mt-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <Label className="text-secondary-light">
                          Nome Completo *
                        </Label>
                        <Input
                          value={newAttendant.name}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (file) {
                              if (file.size > 5 * 1024 * 1024) {
                                toast({
                                  title: "Arquivo muito grande (máximo 5MB)",
                                  variant: "destructive",
                                });
                                return;
                              }
                        ...
                        <Button
                          ...
                          onClick={() =>
                            document
                              .getElementById("new-attendant-image-upload")
                              ?.click()
                          }
                          variant="outline"
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    onClick={async () => {
                      const allData = filteredAndSortedAttendants.map(
                        (attendant: any) => ({
                          name: attendant.name,
                          earnings: attendant.earnings,
                          sales:
                      ...
                      const csv = [
                        ...
                        ...allData.map(
                          (d: any) =>
                            `${d.name},${d.earnings},${d.sales},${d.created}`
                        ),
                      ].join("\n");
                      ...
                      toast({
                        ...
                        description: "CSV com todos os atendentes baixado.",
                      });
                    }}
                    variant="outline"
                  ...
                  <Button
                    onClick={() => {
                      ...
                      toast({
                        ...
                        description:
                          "Informações sincronizadas com o servidor.",
                      });
                    }}
                    variant="outline"
                  ...
                  <Button
                    onClick={() => {
                      ...
                      const active = filteredAndSortedAttendants.filter(
                        (a) => a.status === "active"
                      ).length;
                      const totalSales = filteredAndSortedAttendants.reduce(
                        (sum, a) => sum + parseFloat(a.earnings),
                        0
                      );
...
📅 Gerado em: ${new Date().toLocaleString("pt-BR")}`;
                      ...
                      toast({
                        ...
                        description: "Estatísticas gerais copiadas.",
                      });
                    }}
                    variant="outline"
                  ...
                  <Button
                    onClick={() => {
                      ...
                      if (navigator.share) {
                        ...
                        toast({
                          title: "Mensagem motivacional copiada!",
                          description: "Pronta para compartilhar com a equipe.",
                        });
                      }
                    }}
                    variant="outline"
                ...
                <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                  <div className="flex flex-col sm:flex-row gap-3 flex-1">
                    <div className="relative">
                      <Search
                        size={16}
                        className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-light"
                      />
                      <Input
                        value={attendantSearchQuery}
                        onChange={(e) =>
                          setAttendantSearchQuery(e.target.value)
                        }
                    ...
                    <div className="flex gap-2">
                      <select
                        ...
                        onChange={(e) =>
                          setAttendantSortBy(e.target.value as any)
                        }
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          setAttendantSortOrder(
                            attendantSortOrder === "asc" ? "desc" : "asc"
                          )
                        }
                        className="border-border"
                        title={`Ordenar ${
                          attendantSortOrder === "asc"
                            ? "decrescente"
                            : "crescente"
                        }`}
                      >
                  ...
                  <div className="flex gap-2">
                    <Button
                      variant={
                        attendantViewMode === "cards" ? "default" : "outline"
                      }
                      size="sm"
                    ...
                    <Button
                      variant={
                        attendantViewMode === "table" ? "default" : "outline"
                      }
                      size="sm"
                    ...
                    <Button
                      variant={
                        attendantViewMode === "detailed" ? "default" : "outline"
                      }
                      size="sm"
            ...
            <Card className="bg-card border-border">
              <CardHeader>
                <CardTitle className="text-primary-light flex items-center justify-between">
                  <span>
                    Atendentes Cadastrados ({filteredAndSortedAttendants.length}
                    )
                  </span>
              ...
              <CardContent>
                {attendantsLoading ? (
                  ...
                  <div className="text-center py-12">
                    <Users
                      size={48}
                      className="mx-auto mb-4 text-muted-light opacity-50"
                    />
                    <p className="text-secondary-light">
                      {attendantSearchQuery
                  ...
                  <>
                    ...
                    {attendantViewMode === "cards" && (
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {filteredAndSortedAttendants.map(
                          (attendant: Attendant) => {
                            const stats = getAttendantStats(attendant);
                            return (
                              <div
                                key={attendant.id}
                                className="bg-input/30 border border-border rounded-lg p-4 hover:bg-input/50 transition-colors"
                              >
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="flex gap-1">
                                    <Button
                                      onClick={() =>
                                        handleViewAttendantDetails(attendant)
                                      }
                                      variant="outline"
                                      size="sm"
                                  ...
                                  <div className="flex gap-1">
                                    ...
                                    <Button
                                      onClick={() =>
                                        shareAttendantReport(attendant)
                                      }
                                      variant="outline"
                                      size="sm"
                                      className="flex-1 border-info text-info hover:bg-info hover:text-white"
                                  ...
                                  <div className="flex gap-1 pt-1 border-t border-border/30">
                                    <Button
                                      onClick={() =>
                                        toggleAttendantStatus(attendant.id)
                                      }
                                      variant="outline"
                                      size="sm"
                                      className="flex-1 border-orange-500 text-orange-500 hover:bg-orange-500 hover:text-white"
                                      title={
                                        ((attendant as any).status === "active"
                                          ? "Desativar"
                                          : "Ativar") + " atendente"
                                      }
                                    >
                                      {(attendant as any).status ===
                                      "active" ? (
                                        <UserX size={12} />
                                      ) : (
                                        <UserCheck size={12} />
                                    ...
                                    <Button
                                      onClick={() =>
                                        deleteAttendantMutation.mutate(
                                          attendant.id
                                        )
                                      }
                                      disabled={
                                        deleteAttendantMutation.isPending
                                      }
                                      variant="destructive"
                                      size="sm"
                                      className="flex-1"
                        ...
                        <table className="w-full">
                          <thead>
                            <tr className="border-b border-border">
                              <th className="text-left py-3 px-2 text-secondary-light font-medium">
                                Atendente
                              </th>
                              <th className="text-left py-3 px-2 text-secondary-light font-medium">
                                Vendas Totais
                            ...
                            {filteredAndSortedAttendants.map(
                              (attendant: Attendant) => {
                                ...
                                return (
                                  <tr
                                    key={attendant.id}
                                    className="border-b border-border/50 hover:bg-input/20"
                                  >
                                    ...
                                    <td className="py-3 px-2 text-primary-light">
                                      {stats.totalSales}
                                    </td>
                                    <td className="py-3 px-2 text-primary-light">
                                      {stats.totalAchievements}
                        ...
                        {filteredAndSortedAttendants.map(
                          (attendant: Attendant) => {
                            ...
                            return (
                              <div
                                key={attendant.id}
                                className="bg-input/30 border border-border rounded-lg p-6"
                              >
                                <div className="flex flex-col lg:flex-row gap-6">
                                  ...
                                  <div className="flex-1 grid grid-cols-2 lg:grid-cols-4 gap-4">
                                    ...
                                    <div className="bg-secondary-dark/30 rounded p-3 text-center">
                                      <TrendingUp
                                        size={20}
                                        className="mx-auto mb-1 text-info"
                                      />
                                      <div className="text-lg font-bold text-primary-light">
                                        R$ {stats.averageSale.toFixed(2)}
                                      </div>
                                      <div className="text-xs text-secondary-light">
                                        Média/Venda
                                      </div>
                                    ...
                                    <div className="bg-secondary-dark/30 rounded p-3 text-center">
                                      <Trophy
                                        size={20}
                                        className="mx-auto mb-1 text-danger"
                                      />
                                      <div className="text-lg font-bold text-primary-light">
                                        {stats.totalAchievements}
                                      </div>
              ...
              <CardContent>
                {salesLoading ? (
                  ...
                  <div className="space-y-3">
                    {(sales as any[]).map((sale: Sale) => {
                      const attendant = (attendants as any[]).find(
                        (a: Attendant) => a.id === sale.attendantId
                      );
                      return (
                        <div
                          key={sale.id}
                          className="flex items-center justify-between p-4 border border-border rounded-lg hover:bg-input/20 transition-colors"
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h4 className="text-primary-light font-semibold text-lg">
                                R$ {sale.value}
                              </h4>
                              <p className="text-secondary-light text-sm">
                                {attendant?.name} •{" "}
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent>
                {goalsLoading ? (
                  ...
                  <div className="space-y-2">
                    {(goals as any[]).map((goal: Goal) => {
                      const attendant = (attendants as any[]).find(
                        (a: Attendant) => a.id === goal.attendantId
                      );
                      const progress =
                      ...
                      return (
                        ...
                        >
                          <div className="flex-1">
                            ...
                            <p className="text-secondary-light text-sm">
                              {attendant?.name} - R$ {goal.currentValue} / R${" "}
                              {goal.targetValue}
                              <span
                                className={
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent>
                {achievementsLoading ? (
                  ...
                  <div className="space-y-2">
                    {(achievements as any[]).map((achievement: Achievement) => {
                      const attendant = (attendants as any[]).find(
                        (a: Attendant) => a.id === achievement.attendantId
                      );
                      return (
                        <div
                          key={achievement.id}
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-secondary-light text-sm">
                                {attendant?.name} • {achievement.pointsAwarded}{" "}
                                pontos •
                                {new Date(
                                  (achievement as any).achievedAt
                                ).toLocaleDateString("pt-BR")}
                              </p>
          ...
          <TabsContent value="dragdrop" className="space-y-6">
            <Card className="bg-card border-border">
              <CardHeader>
                ...
                <p className="text-secondary-light">
                  Arraste e solte para reordenar os atendentes
                </p>
              </CardHeader>
              <CardContent>
                {attendantsLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-secondary-light mt-2">
                      Em breve você poderá reorganizar os atendentes arrastando.
                    </p>
                  </div>
                )}
              </CardContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setNewGoal({ ...newGoal, attendantId: e.target.value })
                }
                className="bg-input border-border text-primary-light w-full p-2 rounded"
              >
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={
                  !newAchievement.attendantId ||
                  !newAchievement.title ||
                  !newAchievement.pointsAwarded ||
      ...
      <Dialog
        open={showAttendantDetails}
        onOpenChange={setShowAttendantDetails}
      >
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {(sales as any[])
                    .filter(
                      (sale: Sale) => sale.attendantId === selectedAttendant.id
                    )
                    .slice(0, 5)
                    .map((sale: Sale) => (
                      <div
                        key={sale.id}
                        className="flex justify-between items-center p-3 bg-input/20 rounded border border-border"
                      >
                        <span className="text-primary-light font-medium">
                          R$ {sale.value}
                        </span>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {(goals as any[])
                    .filter(
                      (goal: Goal) =>
                        goal.attendantId === selectedAttendant.id &&
                        goal.isActive
                    )
                    .map((goal: Goal) => {
                      const progress =
                      ...
                      return (
                        <div
                          key={goal.id}
                          className="p-4 bg-input/20 rounded border border-border"
                        >
                          <div className="flex justify-between items-center mb-2">
                            <h5 className="font-medium text-primary-light">
                              {goal.title}
                            </h5>
                  ...
                  {(achievements as any[])
                    .filter(
                      (achievement: Achievement) =>
                        achievement.attendantId === selectedAttendant.id
                    )
                    .slice(0, 4)
                    .map((achievement: Achievement) => (
                      <div
                        key={achievement.id}
                        className="flex items-center gap-3 p-3 bg-input/20 rounded border border-border"
                      >
                        <div
                        ...
                        <div className="flex-1">
                          <h6 className="font-medium text-primary-light text-sm">
                            {achievement.title}
                          </h6>
                          <p className="text-xs text-secondary-light">
                            {achievement.pointsAwarded} pontos •{" "}
                            {new Date(
                              (achievement as any).achievedAt
                            ).toLocaleDateString("pt-BR")}
                          </p>
          ...
          <div className="space-y-6">
            ...
            {editingAttendant && (
              <div className="bg-input/20 rounded-lg p-4 border border-border">
                ...
                <div className="flex items-center gap-4 mb-3">
                  ...
                  />
                  <div className="flex-1">
                    <h4 className="text-lg font-semibold text-primary-light">
                      {editingAttendant.name}
                    </h4>
                    <p className="text-success font-bold text-xl">
                      R$ {editingAttendant.earnings}
                    </p>
                    <p className="text-secondary-light text-sm">
                      Cadastrado em{" "}
                      {new Date(
                ...
                <div className="grid grid-cols-3 gap-3 mt-4">
                    ...
                    <div className="text-primary-light font-bold">
                      {
                        (sales as any[]).filter(
                          (sale: Sale) =>
                            sale.attendantId === editingAttendant.id
                        ).length
                      }
                    </div>
                    ...
                    <div className="text-primary-light font-bold">
                      {
                        (goals as any[]).filter(
                          (goal: Goal) =>
                            goal.attendantId === editingAttendant.id &&
                            goal.isActive
                        ).length
                      }
                    </div>
                  ...
                  <div className="text-center bg-secondary-dark/30 rounded p-2">
                    <div className="text-primary-light font-bold">
                      {
                        (achievements as any[]).filter(
                          (achievement: Achievement) =>
                            achievement.attendantId === editingAttendant.id
                        ).length
                      }
                    </div>
                    <div className="text-xs text-secondary-light">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        <Upload
                          size={24}
                          className="mx-auto mb-2 text-secondary-light"
                        />
            ...
            <div className="flex gap-3 pt-4 border-t border-border">
              <Button
                ...
                disabled={
                  !editAttendantData.name || updateAttendantMutation.isPending
                }
                className="bg-success text-white hover:bg-success-dark flex-1"
              >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-primary-light font-medium">
                    {(attendants as any[]).find(
                      (a: Attendant) => a.id === selectedSale.attendantId
                    )?.name || "Desconhecido"}
                  </span>
                </div>
                <div className="flex justify-between items-center">
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setNewSale({ ...newSale, attendantId: e.target.value })
                }
                className="w-full bg-input border-border text-primary-light px-3 py-2 rounded"
              >
                <option value="">Selecione um atendente</option>
            ...
            <div>
              ...
              <Input
                ...
                value={newSale.value}
                onChange={(e) =>
                  setNewSale({ ...newSale, value: e.target.value })
                }
                placeholder="0.00"
                className="bg-input border-border text-primary-light"
              />
